...
inline constexpr std::array<std::array<Bitboard, kBoardSize>, 2> kPawnAttacks = []() {
  ...
  for (std::size_t i = 0; i < kBoardSize; ++i) {
    ...
    attacks[0][i] = ((pawn_coord & internal::kNotAFile) << 7) | ((pawn_coord & internal::kNotHFile) << 9);//white
    attacks[1][i] = ((pawn_coord & internal::kNotHFile) >> 7) | ((pawn_coord & internal::kNotAFile) >> 9);//black
  }
...
std::pair<Bitboard, uint8_t> GetRookParams(const int square) {
  Bitboard mask = 0;
...
inline const std::array<std::array<Bitboard, 4096>, kBoardSize> kRookAttacks = []() {
  ...
  for (std::size_t sq = 0; sq < kBoardSize; ++sq) {
    auto [mask, bits] = GetRookParams(sq);
    for (std::size_t b = 0, config = (1 << bits); b < config; ++b) {
      Bitboard occupied = GetRookOcuppied(b, mask);
      std::size_t index = ((occupied * kRookMagicNumbers[sq]) >> (kBoardSize - bits));
      attacks[sq][index] = GetRookAttackMask(sq, occupied);
    }
  }
...
// inline constexpr std::array<Bitboard, kBoardSize> kBishopMagicBitboards = 

// inline constexpr std::array<Bitboard, kMxCastles> kEmptySquareCastles = {
